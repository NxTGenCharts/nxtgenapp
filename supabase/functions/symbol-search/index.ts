...
// Exchanges TradingView tags as crypto/forex venues even when the raw
// `type` field it returns is something generic like "spot" rather
// than the short "crypto"/"forex" strings the rest of this function
// expects. Extend these lists if another exchange shows the same gap.
const CRYPTO_EXCHANGES = new Set(["BINANCE", "COINBASE", "BITSTAMP", "KRAKEN", "BYBIT", "OKX", "BITFINEX", "HUOBI", "KUCOIN", "GEMINI", "CRYPTOCOM", "BITGET"]);
const FOREX_CFD_EXCHANGES = new Set(["FOREXCOM", "OANDA", "FXCM", "ICMARKETS", "PEPPERSTONE", "SAXO"]);

// TradingView's raw `type` isn't always one of its own documented
// short values ("stock", "forex", "crypto", "index", ...) — for
// exchange-listed crypto pairs it often comes back as "spot" instead
// (this is what caused BTCUSDT@Binance to silently not switch Market
// to Crypto). Fall back to the exchange itself when the raw type
// isn't one we already recognize.
const KNOWN_TYPES = new Set(["forex", "crypto", "stock", "index", "commodity", "fund", "cfd", "futures"]);
function normalizeType(rawType: string | null, exchange: string | null): string | null {
  const t = (rawType || "").toLowerCase();
  if (KNOWN_TYPES.has(t)) return t;
  if (exchange && CRYPTO_EXCHANGES.has(exchange)) return "crypto";
  if (exchange && FOREX_CFD_EXCHANGES.has(exchange)) return "forex";
  return t || null; // unrecognized — surfaced as-is rather than guessed; Market just won't auto-switch for it
}
...
async function fetchTradingView(query: string): Promise<{ results: UnifiedResult[]; available: boolean }> {
  try {
    ...
    const results: UnifiedResult[] = rows.map((r: any) => {
      const exchange = (r.exchange || r.provider_id || "").toString().toUpperCase() || null;
      return {
        symbol: normSymbol(stripHtml(r.symbol)),
        description: stripHtml(r.description),
        exchange,
        type: normalizeType(r.type, exchange),
        country: r.country || null,
